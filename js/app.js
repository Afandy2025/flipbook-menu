(function () {
    ...
    var changePdfBtn = document.getElementById('change-pdf-btn');
    var hint = document.getElementById('hint');

    // ─── State ───
    var pages = [], currentPage = 0, totalPages = 0;
    var pageW = 0, pageH = 0, dpr = 1, pageRatio = 21 / 29.7;
    var isDragging = false, isAnimating = false, flipDirection = 0;
    var cornerOrigin = { x: 0, y: 0 }, pointerPos = { x: 0, y: 0 };
    var audioCtx = null;
    ...
    function setupUploadEvents() {
        ...
        uploadZone.addEventListener('keydown', function (e) { if (e.key === 'Enter' || e.key === ' ') fileInput.click(); });
        fileInput.addEventListener('change', function (e) { if (e.target.files[0]) handleFileUpload(e.target.files[0]); });
    ...
    function handleFileUpload(file) {
        ...
        fetch('/api/upload', { method: 'POST', body: formData })
            ...
            .then(function (data) {
                var fullUrl = window.location.origin + '/view/' + data.id;
                shareUrl.value = fullUrl;
                shareBanner.classList.remove('hidden');
                history.pushState(null, '', '/view/' + data.id);
                return loadPDFFromServer(data.id);
            })
            .catch(function (err) {
    ...
    function loadPDFFromServer(id) {
        progressText.textContent = 'Loading PDF…';
        showProgress();

        var pdfUrl = '/api/pdf/' + id;
        return pdfjsLib.getDocument(pdfUrl).promise.then(function (pdfDoc) {
            ...
            return pdfDoc.getPage(1).then(function (firstPage) {
                ...
                for (var i = 1; i <= numPages; i++) {
                    (function (pageNum) {
                        chain = chain.then(function () {
                            progressDetail.textContent = 'Page ' + pageNum + ' of ' + numPages;
                            progressBarFill.style.width = ((pageNum - 1) / numPages * 100) + '%';
                ...
                return chain.then(function () {
                    ...
                    var match = window.location.pathname.match(/^\/view\/([a-f0-9]+)$/i);
                    if (match) {
    ...
    function renderPDFPage(page) {
        var targetWidth = Math.min(1600, window.innerWidth * 2);
        ...
        return page.render({ canvasContext: offscreen.getContext('2d'), viewport: viewport }).promise.then(function () {
    ...
    function showFlipbook() {
        ...
        setTimeout(function () {
            ...
            var px = pageW * 0.78, py = pageH * 0.82;
            (function step(now) {
                var t = Math.min((now - t0) / dur, 1);
                if (t < 0.45) {
                    var e = Math.sin((t / 0.45) * Math.PI / 2);
                    pointerPos = { x: pageW + (px - pageW) * e, y: pageH + (py - pageH) * e };
                } else {
                    var e2 = 1 - Math.pow(1 - (t - 0.45) / 0.55, 2);
                    pointerPos = { x: px + (pageW - px) * e2, y: py + (pageH - py) * e2 };
                }
                render();
    ...
    function computeFold(corner, pointer) {
        var dx = pointer.x - corner.x, dy = pointer.y - corner.y, dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < 1) return null;
        return { mx: (corner.x + pointer.x) / 2, my: (corner.y + pointer.y) / 2, fdx: -dy / dist, fdy: dx / dist, fnx: -dx / dist, fny: -dy / dist, dist: dist };
    }

    function constrainPointer(corner, ptr) {
        var px = ptr.x, py = ptr.y, m = 30;
        if ((corner.x + px) / 2 < -m) px = -2 * m - corner.x;
        if ((corner.x + px) / 2 > pageW + m) px = 2 * (pageW + m) - corner.x;
        if ((corner.y + py) / 2 < -m) py = -2 * m - corner.y;
        if ((corner.y + py) / 2 > pageH + m) py = 2 * (pageH + m) - corner.y;
        return { x: px, y: py };
    }

    function clipFold(f, cornerSide, curv) {
        var sign = cornerSide ? 1 : -1, ext = (pageW + pageH) * 2;
        var lx1 = f.mx - f.fdx * ext, ly1 = f.my - f.fdy * ext;
        var lx2 = f.mx + f.fdx * ext, ly2 = f.my + f.fdy * ext;
        var ox = f.fnx * ext * sign, oy = f.fny * ext * sign;
        var cpx = f.mx - f.fnx * curv, cpy = f.my - f.fny * curv;
        ctx.beginPath(); ctx.moveTo(lx1, ly1);
        if (curv > 0.5) ctx.quadraticCurveTo(cpx, cpy, lx2, ly2);
        else ctx.lineTo(lx2, ly2);
        ctx.lineTo(lx2 + ox, ly2 + oy); ctx.lineTo(lx1 + ox, ly1 + oy);
    ...
    function renderCurl() {
        var cPtr = constrainPointer(cornerOrigin, pointerPos);
        var f = computeFold(cornerOrigin, cPtr);
        if (!f) { drawPage(currentPage); return; }
        var diag = Math.sqrt(pageW * pageW + pageH * pageH);
        var progress = Math.min(1, f.dist / diag);
        var curv = Math.sin(progress * Math.PI) * pageW * 0.06;
        var revIdx = flipDirection === 1 ? currentPage + 1 : currentPage - 1;

        // Layer 1: Revealed page (corner side)
        ctx.save(); ctx.beginPath(); ctx.rect(0, 0, pageW, pageH); ctx.clip();
        clipFold(f, true, curv); drawPage(revIdx);
        var sw = Math.min(70, pageW * progress * 0.25);
        if (sw > 2) {
            var sg = ctx.createLinearGradient(f.mx + f.fnx, f.my + f.fny, f.mx + f.fnx * sw, f.my + f.fny * sw);
            var sa = Math.min(0.45, progress * 0.55);
            sg.addColorStop(0, 'rgba(0,0,0,' + sa + ')');
            sg.addColorStop(0.35, 'rgba(0,0,0,' + sa * 0.35 + ')');
            sg.addColorStop(1, 'rgba(0,0,0,0)');
            ctx.fillStyle = sg; ctx.fillRect(-50, -50, pageW + 100, pageH + 100);
        }
        ctx.restore();

        // Layer 2: Current page (kept side)
        ctx.save(); ctx.beginPath(); ctx.rect(0, 0, pageW, pageH); ctx.clip();
        clipFold(f, false, curv); drawPage(currentPage); ctx.restore();

        // Layer 3: Curl back (kept side, reflected)
        ctx.save(); ctx.beginPath(); ctx.rect(0, 0, pageW, pageH); ctx.clip();
        clipFold(f, false, curv);
        ctx.save(); reflectAcrossFold(f);
        ctx.beginPath(); ctx.rect(0, 0, pageW, pageH); ctx.clip();
        clipFold(f, true, curv);
        ctx.fillStyle = '#efe9df'; ctx.fillRect(0, 0, pageW, pageH);
        ctx.globalAlpha = 0.05; drawPage(currentPage); ctx.globalAlpha = 1;
        ctx.restore();
        var cw = Math.max(20, f.dist * 0.5);
        var cg = ctx.createLinearGradient(f.mx - f.fnx, f.my - f.fny, f.mx - f.fnx * cw, f.my - f.fny * cw);
        cg.addColorStop(0, 'rgba(0,0,0,0.2)'); cg.addColorStop(0.1, 'rgba(255,255,255,0.06)');
        cg.addColorStop(0.3, 'rgba(0,0,0,0.04)'); cg.addColorStop(0.7, 'rgba(255,255,255,0.02)');
        cg.addColorStop(1, 'rgba(0,0,0,0.1)');
        ctx.fillStyle = cg; ctx.fillRect(-50, -50, pageW + 100, pageH + 100);
        ctx.restore();

        // Layer 4: Fold edge
        if (progress > 0.01) {
            ctx.save(); ctx.beginPath(); ctx.rect(0, 0, pageW, pageH); ctx.clip();
            var ext = (pageW + pageH) * 2;
            var cpx = f.mx - f.fnx * curv, cpy = f.my - f.fny * curv;
            ctx.beginPath();
            ...
            ctx.lineWidth = Math.min(1.8, progress * 2.5); ctx.stroke();
            ctx.restore();
        }
        ...
        if (progress > 0.015) {
            ctx.save(); ctx.beginPath(); ctx.rect(0, 0, pageW, pageH); ctx.clip();
            clipFold(f, false, curv);
            var hw = Math.min(25, pageW * progress * 0.12);
            var hg = ctx.createLinearGradient(f.mx - f.fnx, f.my - f.fny, f.mx - f.fnx * hw, f.my - f.fny * hw);
            hg.addColorStop(0, 'rgba(255,255,255,' + Math.min(0.14, progress * 0.18) + ')');
            hg.addColorStop(1, 'rgba(255,255,255,0)');
            ctx.fillStyle = hg; ctx.fillRect(-50, -50, pageW + 100, pageH + 100);
            ctx.restore();
        }
    }
    ...
    function onPointerMove(e) {
        if (!isDragging) {
            var p = getPointerPos(e), ez = pageW * 0.30;
            canvas.style.cursor = ((p.x > pageW - ez && currentPage < totalPages - 1) || (p.x < ez && currentPage > 0)) ? 'grab' : 'default';
            return;
        }
    ...
    function onPointerUp() {
        ...
        var f = computeFold(cornerOrigin, cPtr);
        var diag = Math.sqrt(pageW * pageW + pageH * pageH);
        var progress = f ? Math.min(1, f.dist / diag) : 0;
    ...
    function animateComplete() {
        isAnimating = true;
        var startX = pointerPos.x, startY = pointerPos.y;
        var targetX = cornerOrigin.x === pageW ? -pageW * 0.35 : pageW * 1.35;
        var targetY = cornerOrigin.y;
        playFlipSound();
        var t0 = performance.now(), dur = 480;
        function step(now) {
            var t = Math.min((now - t0) / dur, 1), e = 1 - Math.pow(1 - t, 3);
            pointerPos = { x: startX + (targetX - startX) * e, y: startY + (targetY - startY) * e };
            render();
            if (t < 1) requestAnimationFrame(step);
    ...
    function animateSnapBack() {
        isAnimating = true;
        var startX = pointerPos.x, startY = pointerPos.y;
        var targetX = cornerOrigin.x, targetY = cornerOrigin.y;
    ...
    function bindFlipbookEvents() {
        ...
        navLeft.addEventListener('click', function () { autoFlip(-1); });
        navRight.addEventListener('click', function () { autoFlip(1); });
        ...
        window.addEventListener('resize', function () {
            if (!flipbookContainer.classList.contains('hidden')) { calculateDimensions(); render(); }
        });
    }
    ...
    // Check if we're on a /view/:id URL → auto-load
    var match = window.location.pathname.match(/^\/view\/([a-f0-9]+)$/i);
    if (match) {
        uploadScreen.classList.add('hidden');
        loadPDFFromServer(match[1]);
    }
})();
